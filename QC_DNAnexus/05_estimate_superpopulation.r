...
project_onto_ref_PCs <- function(bed.ref, obj.bed, n_PCs = 20, n_PC_plot = 8,
  outdir = "./", filename = "output", strand_flip=FALSE) {
...
ukb_fam_lindgren <- "genotype/ukb22418_cX_b0_v2.fam"
ukb_fam <- "genotype/ukb22418_cX_b0_v2_for_plink.fam"
fwrite(fread('genotype/ukb22418_cX_b0_v2.fam') %>% mutate(V6=-9), file='genotype/ukb22418_cX_b0_v2_for_plink.fam', sep=' ', col.names=FALSE)

# First, generate cleaned up genotype data
write(sapply(1:22, function(chr) {
  c(paste0("genotype/ukb22418_c", chr, "_b0_v2.bed"),
    paste0("genotype/ukb22418_c", chr, "_b0_v2.bim"),
    ukb_fam)
}), 'plink_ukb_filepaths.txt', ncolumns = 3)

# If this dies, it's likely due to memory. Requires at least 10 slots on qe.
snp_plinkQC(
  plink.path = './plink',
  prefix.in = 'plink_ukb_filepaths.txt',
  file.type = "--merge-list",
  prefix.out = "genotype/ukb_autosomes_combined",
  geno = 0.01,
  autosome.only = TRUE
)

obj.bed <- bed('genotype/ukb_autosomes_combined.bed')
...
download.file("ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/technical/working/20130606_sample_info/20130606_g1k.ped",
               destfile = "data/20130606_g1k.ped")
dt_pop <- fread("data/20130606_g1k.ped") %>% 
...
fwrite(dt_classify, file = "superpopulation_labels.tsv", sep='\t')
